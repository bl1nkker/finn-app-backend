...
class UserFacility(models.Model):
    ...
    def __str__(self):
        return f'{self.user.full_name} | {self.facility.name} | {self.role.name}'
...
class Signal(models.Model):
    signal_types = [
        ('1', 'Отрицательный остаток'),
        ('2', 'Проверить документы'),
        ('3', 'Несоответствие наличных денег'),
    ]
    type_signal = models.CharField(
        default='1', max_length=255, choices=signal_types)
    facility = models.ForeignKey(
        Facility, default=None, blank=True, null=True, on_delete=models.CASCADE)
...
class WorkHours(models.Model):
    ...
    employee = models.ForeignKey(
        Employee, on_delete=models.CASCADE, related_name='work_hours',)
...
class Deduction(models.Model):
    ...
    type_deduction = models.CharField(
        default='1', max_length=255, choices=deduction_types)
    date = models.DateField(default=now)
    employee = models.ForeignKey(
        Employee, on_delete=models.CASCADE, related_name='deductions',)
    amount = models.IntegerField(default=0)
    commentary = models.TextField(max_length=10000, default="")
